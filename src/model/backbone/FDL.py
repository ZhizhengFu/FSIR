...
# modify from https://github.com/eezkni/FDL/blob/main/FDL_pytorch/FDL.py
class FDL_loss(nn.Module):
    ...
    def forward_once(self, x, y, idx):
        ...
        projx = F.conv2d(x, rand, stride=self.stride)  # type: ignore
        projx = projx.reshape(projx.shape[0], projx.shape[1], -1)
        projy = F.conv2d(y, rand, stride=self.stride)  # type: ignore
    ...
    def forward(self, x, y):
        ...
        score = score.mean()  # mean within batch   # type: ignore
